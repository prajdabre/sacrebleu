"""The implementation of bytF (Dabre 2025) and bytF++ (Dabre 2025) metrics."""
...
class BYTFScore(Score):
    """A convenience class to represent bytF scores.

    :param score: The bytF (bytF++) score.
    :param byte_order: The byte n-gram order.
    :param char_order: The character n-gram order. If equals to 6, the metric is referred to as bytF++.
    :param word_order: The word n-gram order. If equals to 2, the metric is referred to as bytF++.
    :param beta: Determine the importance of recall w.r.t precision.
    """
    def __init__(self, score: float, byte_order: int, char_order: int, word_order: int, beta: int):
        """`BYTFScore` initializer."""
        self.beta = beta
        self.byte_order = byte_order
        self.char_order = char_order
        self.word_order = word_order

        # Add + signs to denote bytF+ variant
        name = f'bytF{self.beta}' + '+' * self.char_order + '+' * self.word_order

        super().__init__(name, score)


class BYTF(Metric):
    """Computes the bytF(++) metric given hypotheses and references.

    :param byte_order: Byte n-gram order.
    :param char_order: Character n-gram order. If equals to 6, the metric is referred to as bytF++.
    ...
    :param whitespace: If `True`, include whitespaces when extracting byte and character n-grams.
    ...
    # Maximum byte n-gram order to take into account
    BYTE_ORDER = 10

    # Maximum character n-gram order to take into account
    CHAR_ORDER = 0

    # bytF+ additionally takes into account some of the word n-grams
    WORD_ORDER = 0

    # Defaults to 2 (per http://www.aclweb.org/anthology/W16-2341)
    BETA = 2 ## Needs tuning. TODO.
    ...
    def __init__(self, byte_order: int = BYTE_ORDER,
        ...
        self.char_order = char_order
        self.word_order = word_order
        self.order = self.byte_order + self.char_order + self.word_order
    ...
    def _remove_punctuation(self, sent: str) -> List[str]:
        """Separates out punctuations from beginning and end of words for bytF.
        Adapted from https://github.com/m-popovic/bytF
    ...
    def _compute_f_score(self, statistics: List[int]) -> float:
        ...
        :param statistics: A flattened list of 3 * (`byte_order + char_order + word_order`)
        ...
        for i in range(self.order):
            ...
            # bytF++.py style EPS smoothing (also used by Moses and NLTK)
    ...
    def _compute_score_from_stats(self, stats: List[int]) -> BYTFScore:
        ...
        """
        return BYTFScore(
            self._compute_f_score(stats), self.byte_order, self.char_order,
            self.word_order, self.beta)

    def _aggregate_and_compute(self, stats: List[List[int]]) -> BYTFScore:
    ...
    def _extract_reference_info(self, refs: Sequence[str]) -> Dict[str, List[List[Counter]]]:
        """Given a list of reference segments, extract the byte, character and word n-grams.
        ...
        for ref in refs:
            ...
            stats = extract_all_byte_ngrams(ref, self.byte_order, self.whitespace)
    ...
    def _compute_segment_statistics(
        ...
        # extract byte n-grams
        all_hyp_ngrams = extract_all_byte_ngrams(
            hypothesis, self.byte_order, self.whitespace)
        
        # extract character n-grams
        if self.char_order > 0:
            _range = range(1, self.char_order + 1)
            all_hyp_ngrams.extend([extract_char_ngrams(hypothesis, n, self.whitespace) for n in _range])

        # Check bytF+ mode to see if we'll add word n-grams as well
        if self.word_order > 0:
